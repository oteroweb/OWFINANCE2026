...
### Paso 1: Visual Regression
```bash
# Tomar screenshots de cada viewport
# Comparar con Stitch HTMLs
# Usar herramienta como Percy.io o manual comparison
```

### Paso 2: Responsive Testing
```bash
# Usar Chrome DevTools device emulation
# Probar en cada breakpoint listado arriba
# Validar touch targets (mínimo 48x48px)
```

### Paso 3: Device Testing
```bash
# Si hay dispositivos físicos disponibles
# Probar en Safari iOS
# Probar en Chrome Android
# Revisar notch/safe area rendering
```

### Paso 4: Accessibility Audit
```bash
# Usar axe DevTools extension
# Correr Lighthouse audit
# Manual keyboard navigation
# Screen reader testing (si es posible)
```

### Paso 5: Performance Audit
```bash
# Lighthouse Desktop y Mobile
...
1. Deployar a `appfinanzasdev.blockshift.website/app/`
2. Verificar en dispositivos reales
3. Recibir feedback del equipo
4. Iterar si hay issues
...
1. **Crear issue en Github/Notion** con:
...
2. **Priorizar:**
   - Blocker: No permite continuar
   - High: Afecta usabilidad
   - Medium: Mejora necesaria
   - Low: Nice to have

3. **Crear sub-tickets** si es necesario:
   - OFB-004.1: Fix header responsiveness
   - OFB-004.2: Improve FAB sizing
   - etc.
...
Task es considerada **DONE** cuando:

- [x] Todos items del checklist verificados
- [x] Cero blockers encontrados
...
- **OFB-001:** Header (parte del testing)
- **OFB-002:** Bottom Navigation (parte del testing)
- **OFB-003:** Layout Integration (parte del testing)
...
- **Stitch HTMLs:** `html-exports/STITCH_LAYOUTS_LITE_*.html`
